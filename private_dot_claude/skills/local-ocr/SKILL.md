...
behind Preview's copy-paste. It runs fully local, costs nothing, needs no API key,
and ships a prebuilt binary via npm — no Xcode or Swift toolchain. Despite the name
there is no brew tap; `npx --yes mac-ocr` is the working invocation.

Use it for bulk triage. Fall back to model-based OCR (the Read tool) only for images
mac-ocr cannot parse — handwriting, heavy stylization — or when the task needs
layout or semantic understanding beyond raw text.

## Spaced file paths

The Bash tool's sandbox has an intermittent bug with literal spaced paths:
`"Screenshot 2026-01-01 at 9.35.00 AM.png"` fails to open even when the path is
correct, and `dangerouslyDisableSandbox: true` does not help.
